...
//!
//! ```ignore
//! use charter::input::{InputMode, InputContext};
...
use crate::events::{
    AppEvent, DataEvent, DrawingEvent, InputMode, ModeEvent, Modifiers, Panel, ReplayEvent,
    UiEvent, ViewEvent,
};
    ...
    fn map_key_selecting(&self, key: KeyCode) -> Option<AppEvent> {
        match key {
            ...
            KeyCode::Delete | KeyCode::Backspace => {
                // Note: The actual deletion event will need the DrawingId,
                // which the caller will need to provide
                Some(AppEvent::Drawing(DrawingEvent::Cancelled)) // Placeholder - actual delete needs ID
            }
